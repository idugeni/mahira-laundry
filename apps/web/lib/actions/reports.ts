...
export async function generateReportData(type: ReportType, dateRange?: { from: string; to: string }) {
  ...
  const reportRows = orders.map((order: any) => ({
    ...
    "Total": order.total,
    "Items": order.order_items?.map((i: any) => `${i.service_name} (${i.quantity} ${i.unit})`).join(", ")
  }));
  ...
  return {
    ...
    summary: {
      ...
      outletStats: Object.entries(outletStats).map(([name, stats]) => ({ name, ...stats }))
    }
  };
}
