...
const MitraSchema = z.object({
  ...
  outletSlug: z.string().min(3).regex(/^[a-z0-9-]+$/, "Slug must be lowercase and only contain numbers and hyphens"),
...
export async function registerMitra(formData: any) {
  try {
    ...
    if (outletError) {
      if (outletError.code === "23505") throw new Error("Slug outlet sudah digunakan. Coba slug lain.");
      throw outletError;
    }
    ...
    return { success: false, error: error.message || "Gagal mendaftarkan mitra baru." };
  }
}
