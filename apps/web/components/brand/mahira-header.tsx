            ...
            {!loading &&
              (user ? (
                <Link
                  href={dashboardHref}
                  onClick={() => setIsOpen(false)}
                  className="flex items-center justify-between w-full px-5 py-4 bg-slate-900 text-white rounded-2xl font-bold shadow-xl shadow-slate-200"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-brand-accent text-slate-900 flex items-center justify-center font-bold">
                      {getInitials(profile?.full_name as string)}
                    </div>
