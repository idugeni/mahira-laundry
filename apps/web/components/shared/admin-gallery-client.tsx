			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-slate-500 mt-1">
						Tambahkan bukti kualitas layanan Mahira Laundry.
					</p>
				</div>
			</div>
			...
			<div className="grid lg:grid-cols-3 gap-10">
				...
				<div className="lg:col-span-1">
					<form
						...
						className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm sticky top-24"
					>
						...
						<div className="space-y-6">
							...
							<div>
								...
								<Select value={category} onValueChange={setCategory}>
									<SelectTrigger className="w-full px-4 py-3 h-[46px] rounded-xl border border-slate-100 focus:ring-brand-primary/20 outline-none transition-all text-sm font-bold">
										<SelectValue placeholder="Pilih Kategori" />
									</SelectTrigger>
									<SelectContent className="rounded-2xl border-slate-100 shadow-2xl">
										{categories.map((c) => (
											<SelectItem key={c} value={c} className="font-bold py-3">
												{c}
											</SelectItem>
										))}
									</SelectContent>
									...
									<div className="relative aspect-video overflow-hidden">
										...
										<div className="absolute top-4 right-4 flex gap-2">
											<button
												type="button"
												onClick={() => {
													toast.info("Fitur Edit Galeri segera hadir.");
												}}
												className="w-10 h-10 rounded-xl bg-white/90 backdrop-blur-sm shadow-xl flex items-center justify-center text-slate-600 hover:bg-brand-primary hover:text-white transition-all shadow-slate-100/20"
											>
