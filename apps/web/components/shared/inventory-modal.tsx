...
import { upsertInventory, restockInventory } from "@/lib/actions/inventory";
import { toast } from "sonner";
import { HiOutlineCube, HiOutlineQrCode, HiOutlineSquares2X2, HiOutlineArchiveBoxArrowDown, HiOutlinePencilSquare, HiOutlineXMark } from "react-icons/hi2";
...
export function InventoryModal({ item, outletId, mode = "create", trigger }: InventoryModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.success) {
      toast.success(
        mode === "restock" ? "Stok berhasil ditambahkan" : 
        mode === "edit" ? "Item diperbarui" : "Item ditambahkan"
      );
      setIsOpen(false);
    } else {
               ...
               <div className="relative flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight font-[family-name:var(--font-heading)]">
                    {mode === "restock" ? `Restock: ${item?.name}` : 
                     mode === "edit" ? "Edit" : "Tambah"} <span className="text-brand-primary">Item</span>
                  </h2>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-5">
                {mode === "restock" ? (
                  <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4 p-6 bg-blue-50/50 rounded-3xl border border-blue-100/50">
                      <div>
                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Stok Saat Ini</p>
                        <p className="text-2xl font-black text-blue-900">{item?.quantity} <span className="text-xs text-blue-400 uppercase tracking-tighter ml-1">{item?.unit}</span></p>
                      </div>
                      <div className="text-right border-l border-blue-100 pl-4">
                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Minimal Stok</p>
                        <p className="text-2xl font-black text-blue-900">{item?.min_stock} <span className="text-xs text-blue-400 uppercase tracking-tighter ml-1">{item?.unit}</span></p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Jumlah Restock ({item?.unit})</label>
                      <div className="relative group/input">
                        ...
                        <input 
                          required type="number" name="restock_amount" autoFocus
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-5">
                      <div className="space-y-2">
                        ...
                        <div className="relative group/input">
                          ...
                          <input 
                            required name="name" defaultValue={item?.name}
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">SKU / Kode Stok</label>
                        <div className="relative group/input">
                          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/input:text-brand-primary transition-colors">
                            <HiOutlineQrCode />
                          </span>
                          <input 
                            name="sku" defaultValue={item?.sku}
                            placeholder="DET-001"
                            className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold placeholder:text-slate-300 outline-none"
                          />
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="space-y-2">
                        ...
                        <div className="relative group/input">
                          ...
                          <input 
                            name="category" defaultValue={item?.category}
                      ...
                      <div className="space-y-2">
                        ...
                        <input 
                          required type="number" name="quantity" defaultValue={item?.quantity || 0}
                      ...
                      <div className="space-y-2">
                        ...
                        <input 
                          required name="unit" defaultValue={item?.unit || "pcs"}
                    ...
                    <div className="grid grid-cols-2 gap-5">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Batas Minimal Stok</label>
                        <input 
                          type="number" name="min_stock" defaultValue={item?.min_stock || 0}
                ...
                <div className="pt-6 flex items-center gap-4">
                  ...
                  >
                    ...
                    {mode === "restock" ? "Konfirmasi Restock" : 
                     mode === "edit" ? "Update Item" : "Simpan Item"}
                  </button>
                </div>
              </form>
            </div>
