    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden bg-white border-b border-slate-100">
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10 text-center">
          ...
          >
            Cari jawaban cepat untuk pertanyaan yang sering ditanyakan mengenai layanan, harga, hingga kemitraan kami.
          </motion.p>
          ...
          >
            <HiOutlineSearch className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 text-xl" />
            <input
              type="text"
          ...
          <div className="lg:w-1/3">
            <div className="sticky top-28 bg-white/70 backdrop-blur-xl p-4 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white flex flex-col gap-2">
              ...
              {faqCategories.map((cat) => (
                ...
                >
                  <span className={`text-xl ${activeCategory === cat.id ? "text-white" : "text-brand-primary"}`}>
                    {cat.icon}
                  </span>
                  <span>{cat.label}</span>
                </button>
          ...
          <div className="lg:w-2/3">
            <div className="space-y-4">
              <AnimatePresence mode="wait">
                {filteredCategories.find(c => c.id === activeCategory)?.questions.map((faq, idx) => (
                  <motion.div
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <span className={`text-lg md:text-xl font-bold transition-colors ${openIndex === idx ? "text-brand-primary" : "text-slate-900"}`}>
                          {faq.q}
                        </span>
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${openIndex === idx ? "bg-brand-primary text-white rotate-180" : "bg-slate-100 text-slate-400 group-hover:bg-slate-200"}`}>
      ...
      <section className="max-w-4xl mx-auto px-4 mt-24">
        <div className="bg-slate-900 rounded-[3rem] p-8 md:p-12 text-center relative overflow-hidden shadow-2xl shadow-slate-900/20">
          ...
          <div className="relative z-10">
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Masih punya pertanyaan lain?</h2>
