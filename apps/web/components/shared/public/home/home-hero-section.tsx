...
export function HomeHeroSection({
	...
	return (
		<section
			ref={containerRef}
			className="relative w-full min-w-0 lg:h-[calc(100vh-80px)] lg:min-h-[660px] flex flex-col justify-center overflow-hidden py-10 lg:py-0 bg-white"
		>
			...
			<div className="w-full mx-auto px-4 sm:px-8 lg:px-12 xl:px-20 relative flex items-center min-w-0">
				<div className="grid lg:grid-cols-[1.45fr_1fr] gap-10 lg:gap-12 items-center w-full min-w-0">
					...
					>
						<motion.div
							...
							className="inline-flex items-center gap-3 px-6 py-2.5 bg-brand-primary/[0.03] border border-brand-primary/20 text-brand-primary rounded-full text-[11px] font-black mb-8 uppercase tracking-[0.3em] transition-all duration-500 hover:bg-brand-primary hover:text-white cursor-default group/badge"
						>
							<span className="text-base animate-pulse flex items-center justify-center group-hover/badge:text-white transition-colors duration-500">
								<HiOutlineShieldCheck />
							</span>
							<span className="tracking-[0.25em]">Trusted Since 2023</span>
						</motion.div>

						<motion.h1
							variants={itemVariants}
							className="text-4xl sm:text-6xl md:text-7xl lg:text-[68px] xl:text-[80px] font-[950] font-[family-name:var(--font-heading)] leading-[0.9] tracking-[-0.05em] text-slate-900 mb-6"
						>
							Laundry Premium.
							<br />
						...
						<motion.div variants={itemVariants} className="w-full lg:max-w-[95%]">
							<p className="text-xl md:text-2xl text-slate-500 leading-relaxed font-medium mb-8 max-w-3xl">
								Kami merawat pakaian kesayangan Anda dengan teknik terbaik dan bahan yang aman bagi
								serat kain. Bersih maksimal, wangi tahan lama, dan siap dipakai kembali dengan rasa
								bangga.
							</p>

							{/* Premium Pillars */}
							<div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-x-8 gap-y-5 mb-10">
								{premiumFeatures.map((feature, i) => (
									<div key={i} className="flex items-center gap-4 group">
										<div className="w-9 h-9 rounded-xl bg-brand-primary/5 text-brand-primary flex items-center justify-center text-xl transition-all duration-500 group-hover:bg-brand-primary group-hover:text-white group-hover:shadow-lg group-hover:shadow-brand-primary/30">
											{feature.icon}
										</div>
										<span className="text-[11px] xl:text-[13px] font-bold text-slate-800 tracking-tight">
											{feature.text}
										</span>
									</div>
								))}
						...
						<motion.a
							href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_CS?.replace(/\D/g, "").replace(/^0/, "62")}?text=${encodeURIComponent("Halo Mahira Laundry, saya ingin memesan layanan laundry. Mohon informasinya.")}`}
							target="_blank"
					...
					<div className="relative py-8 lg:py-0 flex items-center justify-center w-full min-w-0 h-full">
						<div className="relative w-full max-w-[580px] grid grid-cols-6 grid-rows-6 gap-3 sm:gap-4 h-[580px]">
							{(() => {
								...
								const layouts = [
									"col-span-3 row-span-4", // 1: Flagship (Large Tall)
									"col-span-3 row-span-2", // 2: Secondary (Wide)
									"col-span-3 row-span-2", // 3: Wide
									"col-span-2 row-span-2", // 4: Square
									"col-span-2 row-span-2", // 5: Square
								...
								return curatedSelection.map((service, idx) => {
									...
									return (
										<motion.div
											...
											className={`${layout} ${style} rounded-[2rem] overflow-hidden relative p-6 flex flex-col justify-end group cursor-pointer border border-transparent transition-all duration-500`}
										>
											...
											<motion.div
												...
												className={`absolute inset-0 flex items-center justify-center text-[260px] select-none pointer-events-none transition-all duration-700
													${isDark ? "text-white opacity-[0.05] group-hover:opacity-[0.08]" : "text-brand-primary opacity-[0.04] group-hover:opacity-[0.08]"}
											...
											<div className="relative z-20 w-full">
												{/* Small Icon Wrapper */}
												<div
													className={`mb-3 flex items-center justify-center rounded-2xl shadow-sm w-12 h-12 transition-all duration-500 group-hover:scale-110 group-hover:-translate-y-1
													${
														isBrand
												...
												<div className="flex items-center gap-2 mb-2">
													...
													<div className={`h-px w-4 ${isDark ? "bg-white/20" : "bg-slate-200"}`} />
													<div
														className={`text-[8px] font-bold uppercase tracking-widest transition-opacity duration-500
														${isDark ? "text-white/40" : "text-slate-400"}
													`}
													>
														{idx === 0 ? "Certified Elite" : "Handcrafted"}
													</div>
												</div>

												{/* Service Name */}
												<h3
													className={`font-[950] leading-none tracking-tighter transition-all duration-300
													${idx === 0 ? "text-3xl mb-2" : "text-xl"}
													${
														isBrand
															? "text-white"
															: isDark
												...
												{idx === 0 && (
													<p
														className={`text-[11px] font-medium leading-relaxed line-clamp-2 mt-2 transition-all duration-300
														${
															isBrand
																? "text-white/80"
																: isDark
