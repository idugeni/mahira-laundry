...
import { HiOutlinePencilAlt, HiOutlineInformationCircle, HiPlus } from "react-icons/hi";
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 leading-none">Manajemen Layanan</h2>
            <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">Pusat Pengaturan Produk</p>
          </div>
        </div>
        <ServiceModal 
          outletId={outletId} 
        ...
        {services.map((service) => (
          ...
          >
            ...
            <div className="mt-6 pt-6 border-t border-slate-50 flex items-end justify-between">
              <div>
                <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Mulai Dari</span>
                <span className="text-xl font-black text-slate-900 font-mono">
                  {formatIDR(Number(service.price))}
                  <span className="text-xs text-slate-400 font-bold lowercase ml-0.5">/{service.unit}</span>
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${service.is_active ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-slate-300"}`} />
      ...
      {services.length === 0 && (
        <div className="py-32 text-center bg-white rounded-[3rem] border border-slate-100 shadow-sm">
          ...
          <h3 className="text-xl font-black text-slate-900 tracking-tight">Belum Ada Layanan</h3>
          <p className="text-slate-400 font-medium max-w-xs mx-auto mt-2">Daftar layanan Anda akan muncul di sini setelah Anda menambahkannya.</p>
        </div>
      )}
